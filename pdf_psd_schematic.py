...
A,v_bend,a_low,a_high,c = 1, 1e-3, pl_index, 5, 0
...
#lc_pink = Simulate_TK_Lightcurve(BendingPL, (A,v_bend,a_low,a_high,c),RedNoiseL=RedNoiseL, aliasTbin=aliasTbin,randomSeed=RandomSeed,length=Age*tbin,tbin=tbin)
...
print (lc_pink.time.shape, lc_pink.flux.shape)
...
plt.plot(flux, c=lc_color, lw=1.5, alpha=alpha, label="$\mathrm{Variable~(Pink~noise)}$")
